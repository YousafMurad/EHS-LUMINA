-- Simplified SQL Migration for EHS ERP
...
CREATE POLICY "Admin can manage subjects" ON subjects FOR ALL TO authenticated
...
CREATE POLICY "Admin and operators can manage assignments" ON teacher_class_assignments FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role IN ('super_admin', 'admin', 'operator')));

DROP POLICY IF EXISTS "Teachers can view their own assignments" ON teacher_class_assignments;
CREATE POLICY "Teachers can view their own assignments" ON teacher_class_assignments FOR SELECT TO authenticated
USING (teacher_id IN (SELECT t.id FROM teachers t WHERE t.user_id = auth.uid() OR t.email = (SELECT email FROM profiles WHERE id = auth.uid())));

-- Parent students policies
DROP POLICY IF EXISTS "Parents can view their own links" ON parent_students;
CREATE POLICY "Parents can view their own links" ON parent_students FOR SELECT TO authenticated
USING (parent_id = auth.uid());

DROP POLICY IF EXISTS "Admin can manage parent links" ON parent_students;
CREATE POLICY "Admin can manage parent links" ON parent_students FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role IN ('super_admin', 'admin', 'operator')));
...
USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role IN ('super_admin', 'admin', 'teacher')));
...
CREATE POLICY "Parents can view children attendance" ON attendance FOR SELECT TO authenticated
